...
public class GSMArenaImportTask {
			...
			try {
				...
				if (strValue.contains("|") && strValue.contains("::")) {
					...
					String productCode = HbaseUtils.extractValueMapper(
							"product_code", strValue, "([a-zA-Z0-9 -/_.%]+)");
					String rowkey = manufacturer + "_" + productCode;
					if (rowkey.toLowerCase().contains("review")) {
						return;
					}
					...
					if (!skip) {
						p1.add(Bytes.toBytes("manufacturer"), null,
								Bytes.toBytes(manufacturer));
						String prodCatagory = context.getConfiguration().get(
								"productCatagory");
						p1.add(Bytes.toBytes("productCatagory"), null,
								Bytes.toBytes(prodCatagory));
					}
	...
	public static Job configureJob(Configuration conf, String[] args)
		...
		conf.set(HConstants.ZOOKEEPER_CLIENT_PORT, hbaseZookeeperClientPort);

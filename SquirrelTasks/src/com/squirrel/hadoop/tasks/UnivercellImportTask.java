...
public class UnivercellImportTask {
				...
				if (strValue.contains("|") && strValue.contains("::")) {
					...
					if (tableRowKey == null || tableRowKey.length() == 0) {
						...
						if (tableRowKey == null || tableRowKey.length() == 0) {
							...
							//System.out.println(productCode + ">>" + tableRowKey);
						...
						if (strArray.length != 2) {
							...
							if (!(column.length() > 0 && data.length() > 0)) {
								...
								if(column.equalsIgnoreCase("product_href")){
									data = "http://univercell.in"+data;
								}
							p1.add(Bytes.toBytes("univercell"),
